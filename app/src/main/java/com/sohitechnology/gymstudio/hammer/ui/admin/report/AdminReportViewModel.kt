...
class AdminReportViewModel @Inject constructor(
    ...
    private val _state = MutableStateFlow(ReportState(isAdmin = true))
    ...
    fun getReports(clubIds: String, memberIds: String, startDate: String, endDate: String, forceRefresh: Boolean = false) {
        if (!forceRefresh && ReportCache.reports != null && _state.value.reports.isNotEmpty()) {
            return
        }
    ...
    fun getTransactions(clubId: Int, memberId: Int, startDate: String, endDate: String, forceRefresh: Boolean = false) {
        if (!forceRefresh && ReportCache.transactions != null && _state.value.transactions.isNotEmpty()) {
            return
        }
